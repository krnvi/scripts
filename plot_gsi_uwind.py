...
############################################################################################################################################
    ...
    clevs=[-9,-6,-3,-0,3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51]
    ...
    #(cbar.set_label('m/s')) ;
    (plt.title('GSI U wind at model level '+ str(lev_1),fontsize=12,color='k')); 
    savefig('/home/vkvalappil/Data/modelWRF/GSI/gsiout/2016122006_hybrid_02/uwind/uwind_gsi_'+str(lev_1)+'.png', dpi=100);
    plt.close()
###################################################################################################################################
    fig=plt.figure(figsize=(8,8),dpi=100); ax = fig.add_axes([0.1,0.1,0.8,0.8]) ; 
    m = Basemap(projection='mill',llcrnrlat=lat.min(),urcrnrlat=lat.max(),llcrnrlon=lon.min(),urcrnrlon=lon.max(),resolution='l')
    lons, data = m.shiftdata(lon, datain=w_uu, lon_0=None)
    x, y=m(lon,lat) 
    ...
    cbar=m.colorbar(cs,location='right', pad='5%') ; cbar.set_ticks([clevs])
    #(cbar.set_label('m/s')) ;
    (plt.title('WRF U wind at model level '+ str(lev_1),fontsize=12,color='k')); 
    savefig('/home/vkvalappil/Data/modelWRF/GSI/gsiout/2016122006_hybrid_02/uwind/uwind_wrf_'+str(lev_1)+'.png', dpi=100);
    plt.close()  
#########################################################################################################################################    
    fig=plt.figure(figsize=(8,8),dpi=100); ax = fig.add_axes([0.1,0.1,0.8,0.8]) ; 
    m = Basemap(projection='mill',llcrnrlat=lat.min(),urcrnrlat=lat.max(),llcrnrlon=lon.min(),urcrnrlon=lon.max(),resolution='l')
    lons, data = m.shiftdata(lon, datain=d_uu, lon_0=None)
    x, y=m(lon,lat) 
...
#    #cs=m.pcolormesh(x,y,data, cmap=cmap, norm=norm)
    ...
    cbar=m.colorbar(cs,location='right', pad='5%') ; cbar.set_ticks([clevs])
    #(cbar.set_label('m/s')) ;
    (plt.title('DIF U wind at model level '+ str(lev_1),fontsize=12,color='k')); 
    savefig('/home/vkvalappil/Data/modelWRF/GSI/gsiout/2016122006_hybrid_02/uwind/uwind_dif_'+str(lev_1)+'.png', dpi=100);
    plt.close()      
#########################################################################################################################################################################    
    
fig=plt.figure(figsize=(8,8),dpi=100); ax = fig.add_axes([0.1,0.1,0.8,0.8]) ; 
m = Basemap(projection='mill',llcrnrlat=lat.min(),urcrnrlat=lat.max(),llcrnrlon=lon.min(),urcrnrlon=lon.max(),resolution='l')
lons, data = m.shiftdata(lon, datain=d_u, lon_0=None)
x, y=m(lon,lat) 
...
cbar=m.colorbar(cs,location='right', pad='5%') ; cbar.set_ticks([clevs])
#(cbar.set_label('m/s')) ;
(plt.title('DIF U wind at all model level ',fontsize=12,color='k')); 
savefig('/home/vkvalappil/Data/modelWRF/GSI/gsiout/2016122006_hybrid_02/uwind/uwind_adif_.png', dpi=100);
plt.close()       
