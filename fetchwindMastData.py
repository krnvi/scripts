...
def fetchData(url_1):
   ...
   def getData(url):
        data_1=readUrl(url)    
        if data_1.shape[0]==221:    
             print "Data download complete: "        
             data=data_1.reshape(17,13); reqData_1=np.concatenate([np.vstack(data[0,:]).T,data[-16:-1,:]],axis=0)    
        else:
...
def main():
    ...
    url='http://10.11.255.119/?command=TableDisplay&table=Minute&records=16' ; 
