...
main='/home/vkvalappil/Data/oppModel/' ; output=main+'/output/' ; inp=main+'output/output/domain_2/surfaceLevel/'
date=str(sys.argv[1])
########################################################################################################################################
...
#######################################################################################################################################
        ...
        for dte1 in date_req_list[:]:
            ...
            if (os.path.isfile(file_2)):
                ...
                mod_dom_2=pd.read_csv(file_2) ; mod_dom_2=mod_dom_2.iloc[7:31,:] 
                ...
                obs=pd.concat([obs_1,obs_2],axis=0)
...
##################################################################################################################################################################################################        
       ########## Correction for Temperature #######################################################
        ...
        if ((np.sign(bias_tmp) < 0 ).all()) | ((np.sign(bias_tmp) > 0 ).all()):
            ...
            obs_mod_hour_sum_day1=obs_mod_r.groupby(hourt).sum() 
            a = obs_mod_hour_sum_day1.values ;
