...
T2=np.round(A[1:49,5].astype(np.float)).astype(int) ; T10=np.round(A[1:49,6].astype(np.float)).astype(int) ; Time=A[1:49,0]
...
ax.set_ylabel('Dewpoint Temperature (Deg)',color='blue',fontsize=12)
plt.title('2M & 10M DewPoint Temperature (MI Field Station)',color='black',fontsize=16,y=1.15)

plt.tight_layout(h_pad=3)
savefig('/home/Data/workspace/MI_dewtemp.png', dpi=100);
plt.close(fig)
fig.clf(fig)
