...
T2=np.round(A[1:49,7].astype(np.float)).astype(int) ; T10=np.round(A[1:49,8].astype(np.float)).astype(int) ; Time=A[1:49,0]
...
ax.set_yticks(np.arange(min(T2)-5,max(T2)+5,5.0))
...
ax.set_ylabel('Relative Humidity(%)',color='blue',fontsize=16)
plt.title('2M & 10M Relative Humidity (MI Field Station)',color='black',fontsize=16,y=1.15)

plt.tight_layout(h_pad=3)
savefig('/home/Data/workspace/MI_RH.png', dpi=100);
plt.close(fig)
fig.clf(fig)
