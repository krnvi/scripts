...
date=str(sys.argv[1]) ; 
rad_file=main+'/radiometerAnalysis/NormalDayAnalysis_Feb_28/17022800_17022805.IRT.ASC' ;  
outFile=main+'/radiometerAnalysis/NormalDayAnalysis_Feb_28/17022800_17022805.IRT.png'
###############################################################################################################################################################
...
rad_irt=new_data[:,1].astype(float) ;        #new_data_1[:,1].astype(float) ; #rad_irt=rad_irt.reshape(6,60,60,1) ; m_mean=np.nanmean(rad_irt,axis=3)
...
sns.set(style='ticks') ; pos  = np.arange(1,len(rad_irt)+1,1) ;  
#ax.hold(True)
ax.plot(pos,rad_irt,'blue',linestyle='-',linewidth=2.0,marker='o',markersize=1.0,\
           markeredgewidth=1.0,markerfacecolor='blue',markeredgecolor='blue',label='IR Temp')
...
ax.set_yticks(np.arange(min(np.round(rad_irt).astype(int))-4,max(np.round(rad_irt).astype(int))+4,4.0))
#ax.set_yticks(np.arange(5000,12000,1000.0))
#pos=np.arange(1,len(h_mean),1) ; 
x_lim=pos[::3600] ; ax.set_xticks(x_lim) ;  xTickMarks=new_data_1[::3600,0]
...
titl=' Infrared Temperature' ; plt.title(titl,color='black',fontsize=22,y=1.05)
