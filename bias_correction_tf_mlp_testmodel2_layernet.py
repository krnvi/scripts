...
for ii in range(0,len(date_index)-2):
    ...
    dte=date_index[ii] ; dte1=date_index[ii+2]
    dt_tmp_input=tmp_input[dte:dte1] ;  
...
##########################################################################################################
    ...
    saver = tf.train.import_meta_graph('/home/vkvalappil/Data/workspace/pythonScripts/mlp_bias_cor_3.1.59/mlp_bias_cor.meta') 
    saver.restore(sess,tf.train.latest_checkpoint('/home/vkvalappil/Data/workspace/pythonScripts/mlp_bias_cor_3.1.59/'))
    ...
    rmse_mod=(np.sqrt(mean_squared_error(Y,X['mod.1'])))
    
    rmse_cor.append([rmse_mod,rmse_new])

    tmp_data_new=pd.concat([tmp_input_1['mod.1'],pd.DataFrame(np.round(y_new)),Y,dt_tmp_input['bias'].iloc[24:48].reset_index(drop=True),\
                 bias_new],axis=1)
...
rmse_cor.index=date_index[1:-1] ; rmse_cor.insert(0,'Date',rmse_cor.index)
