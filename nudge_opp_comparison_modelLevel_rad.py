...
main='/home/vkvalappil/Data/modelWRF/nudging/'  ; output=main+'ARW/output/domain_02/modelLevel/stat/RAD/' ; inp=main+'ARW/output/domain_02/modelLevel'

date=str(sys.argv[1])

#date=str(sys.argv[1])
num_hours=23 ; fcs_leaddays=1 ; 
...
for ii in range(0,len(file_date_list[:])):
    ...
    file_1=inp+'/E6nudge_'+date+'/modelLevel_'+file_date_list[ii]+'.csv' ;
...
############################################################################################################################################    
        ...
        obs_file_1=main+'/ARW/obsFiles/OBS_DOMAIN201_'+file_date_list[ii]
        ...
        if (os.path.isfile(obs_file_1)):
            ...
            obs.pressure=obs.pressure/100.0

            otmp=np.array((obs.temperature).apply(pd.to_numeric,errors='coerce'))*units('K')
            orh=np.array((obs.rh).apply(pd.to_numeric,errors='coerce'))
            opress=np.array((obs.pressure).apply(pd.to_numeric,errors='coerce'))*units('millibar')

            omrio=mcalc.mixing_ratio_from_relative_humidity(orh,otmp,opress)
            
            
            
############################################################################################################################       
            x_intp=obs.pressure.values * units.hPa ;  x_i=mod_da_1.pressure.values * units.hPa ; 
...
############################################################################################################################################            
            x_intp=mod_da_1.pressure.values * units.hPa ;    x_i=obs.pressure.values * units.hPa ;
            
            tmp_i=obs.temperature.values ;   rh_i=obs.rh.values ; mr_i=omrio.m/100.0

            #mr_i=obs.mixRatio.values     ;   ws_i=obs.wspd.values
        
            obs_1_temp=mcalc.log_interp(x_intp,x_i,tmp_i)  ;  obs_1_rh=mcalc.log_interp(x_intp,x_i,rh_i)        
            obs_1_mr=mcalc.log_interp(x_intp,x_i,mr_i)  ;
            #mod_da_1_mr=mcalc.log_interp(x_intp,x_i,mr_i)    ;  mod_da_1_ws=mcalc.log_interp(x_intp,x_i,ws_i)                
...
##########################################################################################################################        
            ...
            mod_da_2_temp=mcalc.log_interp(x_intp,x_i_2,tmp_i_2)  ;  mod_da_2_rh=mcalc.log_interp(x_intp,x_i_2,rh_i_2)        
            mod_da_2_mr=mcalc.log_interp(x_intp,x_i_2,mr_i_2)    ;  mod_da_2_ws=mcalc.log_interp(x_intp,x_i_2,ws_i_2)        
                
#############################################################################################################################        
        
            tmp_=pd.concat([mod_da_1['pressure'].reset_index(drop=True),pd.DataFrame(obs_1_temp),mod_da_1['temperature'].reset_index(drop=True),mod_da_2['temperature'].reset_index(drop=True)],axis=1)    
            tmp_.columns=['pressure','obs','Da','no_Da']
            tmp_.index=tmp_.pressure

            plot_line(tmp_.iloc[0:,:],'Temperature', file_date_list[ii]) 
            

            rh_=pd.concat([mod_da_1['pressure'].reset_index(drop=True),pd.DataFrame(obs_1_rh),mod_da_1['rel.Humidity'].reset_index(drop=True),mod_da_2['rel.Humidity'].reset_index(drop=True)],axis=1)    
            rh_.columns=['pressure','obs','Da','no_Da']
            rh_.index=rh_.pressure

            plot_line(rh_.iloc[0:,:],'Relative_humidity', file_date_list[ii]) 


            mr_=pd.concat([mod_da_1['pressure'].reset_index(drop=True),pd.DataFrame(obs_1_mr),mod_da_1['mixRatio'].reset_index(drop=True),mod_da_2['mixRatio'].reset_index(drop=True)],axis=1)    
            mr_.columns=['pressure','obs','Da','no_Da']
            mr_.index=mr_.pressure
