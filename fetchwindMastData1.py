...
def fetchData(url_1):
   def readUrl(url):
        ...
        htm = urll2.urlopen(web).read() ;  cont=bs4.BeautifulSoup(htm,"lxml") ; #cont=cont.get_text() ; 
        #cont=re.sub('<[^<]+?>','',htm) ;

        #cont=re.sub('Table Display','',cont) ; cont=re.sub('Table Name: Minute','',cont) ; cont.encode('UTF8'); 
   ...
   def getData(url):
        data_1=readUrl(url)    
        if data_1.shape[0]==208:    
             print "Data download complete: "        
             data=data_1.reshape(16,13); #reqData_1=np.concatenate([np.vstack(data[0,:]).T,data[-16:-1,:]],axis=0)    
        else:
...
def main():
    ...
    mainpath='/home/Data/masdar_station_data/' ; datapath=mainpath+'metStation/windMast/' ;
    ...
    url='http://10.11.255.119/?command=dataquery&uri=dl:minute&format=html&mode=date-range&p1='+p_1+'&p2='+p_2 ;
