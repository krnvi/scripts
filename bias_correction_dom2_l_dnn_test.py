...
d_t_1='2018-04-30' ; d_t_2='2018-06-01' ; 
...
rmse_hour_avg_day1=pd.concat([rmse_hour_avg_day1]*29)
...
#        return tf.estimator.export.ServingInputReceiver(inputs, inputs)
    ...
    feature_cols = [tf.feature_column.numeric_column(col) for col in norm_X.columns]  
...
############### loading and predict Linear DNN combined regressor
    reg_linear_dnn = tf.estimator.DNNLinearCombinedRegressor(linear_feature_columns=feature_cols,dnn_feature_columns=feature_cols,\
                     dnn_hidden_units=[100,100, 1],model_dir='linear_dnn_bias-cor_full') #linear_dnn_bias-cor
...
################################################################################################################################# 
    ...
    tmp_data_new=pd.concat([tmp_input_1['mod.1'],pd.DataFrame(np.round(y_new_l_dnn)),Y,dt_tmp_input['bias'].iloc[24:48].reset_index(drop=True),\
                 bias_new],axis=1)
