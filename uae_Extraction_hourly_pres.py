...
lev=900
outFile=output+'hourly'+date+'_'+str(lev)+'.csv' ; 
...
tmp=(np.array(grb_f.select(name="Temperature",typeOfLevel='isobaricInhPa',level=lev))) 
RH=(np.array(grb_f.select(name="Relative humidity",typeOfLevel='isobaricInhPa',level=lev))) 
dtmp=(np.array(grb_f.select(name="Dew point temperature",typeOfLevel='isobaricInhPa',level=lev))) 
ugrd=(np.array(grb_f.select(name="U component of wind",typeOfLevel='isobaricInhPa',level=lev))) 
vgrd=(np.array(grb_f.select(name="V component of wind",typeOfLevel='isobaricInhPa',level=lev))) 
...
header='TEHSILID,DATE,TIME(GMT),IST,DAY_SEQUENCE,TEMP,DTEMP,RH,WSPD,WDIR'
header=np.vstack(np.array(header.split(","))).T

fin_mat1=np.concatenate([pre_mat,temp,dtemp,rhum,wdir,wspd],axis=1)
