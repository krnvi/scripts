...
def fetchData(url_1):
   ...
   def getData(url):
        data_1=readUrl(url)    
        if data_1.shape[0]==408:    
             print "Data download complete: "
             data=data_1.reshape(17,24); reqData_1=np.concatenate([np.vstack(data[0,:]).T,data[-16:-1,:]],axis=0)
        else:
...
def main():
    ...
    url='http://10.104.2.31/?command=TableDisplay&table=Weather&records=16' ; 
