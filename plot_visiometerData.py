...
date=str(sys.argv[1]) ; 
vis_file=main+'/visiometerAnalysis/_data/'+date[0:4]+'/'+date[4:6]+'/'+date[0:8]+'/'+date[0:8]+'.csv' ;  
...
ax.plot(pos,vis,'blue',linestyle='-',linewidth=3.0,marker='o',markersize=2.0,\
           markeredgewidth=2.0,markerfacecolor='green',markeredgecolor='black',label='Visisibility')

ax1.plot(pos,ecf,'red',linestyle='-',linewidth=3.0,marker='o',markersize=2.0,\
           markeredgewidth=2.0,markerfacecolor='green',markeredgecolor='black',label='Ext.Coefficient')
...
ax.set_yticks(np.arange(0,max(np.round(vis).astype(int))+4,2))
...
leg=ax.legend(["Visibility"],loc=1,bbox_to_anchor=(-0.3, 0.9, 1., .102),frameon=1)
frame = leg.get_frame() ; frame.set_facecolor('white')
leg=ax1.legend(["Ext. Cofficient"],loc=1,bbox_to_anchor=(0, 0.9, 1., .102),frameon=1)
frame = leg.get_frame() ; frame.set_facecolor('white')

titl=' Visibility & Ext. Coefficient - MI Field Station' ; plt.title(titl,color='black',fontsize=22,y=1.00)
