...
tmpFle=inp+'AirTemperature_'+date+'.nc'
...
visib=rem_data[1:,6] ; cil=rem_data[1:,11] ; cfr=rem_data[1:,10].astype(float)
...
tmpp=np.round(tmp[0::tint]).astype(int) ; tmpp=np.vstack(tmpp.flatten('F'))
...
fin_mat=np.concatenate([pre_mat,wispd,wispd_km,wispd_mi,wispd_kn,wdir,wndshrt,vis1,vis2,skycnd,cil,wcnd,tmpp],axis=1)
...
meantmp=(np.round(req_data_1[:,13].astype(int).mean()).astype(str) +u" \u2103" ).encode('utf-8') 
...
meantmp=(np.round(req_data_2[:,13].astype(int).mean()).astype(str) +u" \u2103" ).encode('utf-8') 
