...
######################################################################################################################################
            ...
            if hgt_r.shape[0] >0:
                ...
                cloud_mask_r_v=np.copy(cloud_mask_r).astype(float)  ; cloud_mask_r_v[invalid_cld]=np.nan
...
#########################################################################################################################################
                    ...
                    ax1.set_xlabel('Second since the start of the granule (Seconds) ')
                    ax1.set_ylabel('Height (m)')
                    basename = os.path.basename(file_name)
                    ax1.set_title('{0}\n{1}'.format(basename,  'CPR Cloud mask'))
                    cbar_ticks = [10,20,30]   ; cbar_ticks_labels = ['','20','Cloudy']
                    cb1 = plt.colorbar(im1,fraction=0.01, cmap=cmap, norm=norm, boundaries=bounds, ticks=cbar_ticks) ; 
                    cb1.ax.set_yticklabels(cbar_ticks_labels, fontsize=8)
###########################################################################################################################################
...
cloudy_date2.columns=['Filename','date']
cloudy_date2.to_csv(file_path+'cloudy_date_'+date[0:4]+'.csv',index=False)
