...
import sys ; import os ; import numpy as np ; import datetime as dt ; from dateutil import tz ; 
import matplotlib.pyplot as plt; import seaborn as sns ;


main='/home/vkvalappil/Data/' ; date=str(sys.argv[1])
output=main+'modelWRF/oppModel/ARW/output/plots/' +date
...
lstFile=main+'modelWRF/oppModel/scripts/master_uae.csv'
...
##################################################Temperature ##########################################################################################    
    l_idx=48 ; outFile=output+'/temperature_2m_'+locNme+date
    data_1=g_req_data[0:l_idx,5].astype(float) ; #data_2=ng_req_data[0:l_idx,5].astype(float) ; 
    obs=rs_req_data[0:l_idx,4].astype(float)
...
##########################################Relative Hum #############################################################    
    ...
    outFile=output+'/rh_2m_'+locNme+date 
    data_1=g_req_data[0:l_idx,7].astype(float) ; #data_2=ng_req_data[0:l_idx,7].astype(float) ; 
    obs=np.round(rs_req_data[0:l_idx,6].astype(float))
...
###################################################Dew temp ######################################################    
    outFile=output+'/dewtemp_2m_'+locNme+date 
    data_1=g_req_data[0:l_idx,6].astype(float) ; #data_2=ng_req_data[0:l_idx,6].astype(float) ; 
    obs=np.round(rs_req_data[0:l_idx,5].astype(float))
...
############################################### Mix ratio #######################################################    
    outFile=output+'/mixr_2m_'+locNme+date
    data_1=g_req_data[0:l_idx,10].astype(float) ; #data_2=ng_req_data[0:l_idx,10].astype(float) ; 
    obs=rs_req_data[0:l_idx,11].astype(float) 
...
#           markeredgewidth=2.0,markerfacecolor='red',markeredgecolor='red',label='NO GSI')
    ...
    ax.set_yticks(np.arange(0,max(data_1)+stp,stp))
...
#################################################################################################################
    ...
    plt.savefig(outFile);
    plt.close(fig) ; fig.clf(fig)  
    del g_req_data,rs_req_data,rs_data ;
#################################################################################################################
