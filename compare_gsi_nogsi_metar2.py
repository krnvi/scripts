...
import os ; import numpy as np ; import datetime as dt ; from dateutil import tz ; 
import matplotlib.pyplot as plt; import seaborn as sns ;


main='/home/vkvalappil/Data/' ; date='2017060506' ; l_idx_s=0 ; l_idx_e=30 ;
output=main+'modelWRF/ARW/output/gsi_plots/' +str(l_idx_s)+'_'+str(l_idx_e)+'hour/cv5_1_J/'+date+'/'
if not os.path.exists(output)   :
    os.makedirs(output)

gsiFile=main+'modelWRF/ARW/output/GFS_ENKF/gsi_opp/newBe/cv5_newbe_1_J/hourly'+date+'_d02.csv'
...
##################################################Temperature ##########################################################################################    
    outFile=output+'/'+str(l_idx_s)+'_'+str(l_idx_e)+'_temperature_2m_'+locNme+date
    data_1=g_req_data[l_idx_s:l_idx_e,5].astype(float) ; data_2=ng_req_data[l_idx_s:l_idx_e,5].astype(float) ; obs=rs_req_data[l_idx_s:l_idx_e,4].astype(float)
    ...
    leg=ax.legend([handle for i,handle in enumerate(handles) if i in display]+[simArtist,anyArtist],
          [label for i,label in enumerate(labels) if i in display],loc=1,bbox_to_anchor=(-0.05, 0.5, 1., .102),frameon=1,fontsize=18)
...
##########################################Relative Hum #############################################################    
    ...
    outFile=output+'/'+str(l_idx_s)+'_'+str(l_idx_e)+'_rh_2m_'+locNme+date 
    data_1=g_req_data[l_idx_s:l_idx_e,7].astype(float) ; data_2=ng_req_data[l_idx_s:l_idx_e,7].astype(float) ; obs=np.round(rs_req_data[l_idx_s:l_idx_e,6].astype(float))
...
###################################################Dew temp ######################################################    
    outFile=output+'/'+str(l_idx_s)+'_'+str(l_idx_e)+'_dewtemp_2m_'+locNme+date 
    data_1=g_req_data[l_idx_s:l_idx_e,6].astype(float) ; data_2=ng_req_data[l_idx_s:l_idx_e,6].astype(float) ; obs=np.round(rs_req_data[l_idx_s:l_idx_e,5].astype(float))
...
############################################### Mix ratio #######################################################    
    outFile=output+'/'+str(l_idx_s)+'_'+str(l_idx_e)+'_mixr_2m_'+locNme+date
    data_1=g_req_data[l_idx_s:l_idx_e,10].astype(float) ; data_2=ng_req_data[l_idx_s:l_idx_e,10].astype(float) ; obs=rs_req_data[l_idx_s:l_idx_e,11].astype(float) 
    ...
    ax.set_yticks(np.arange(scl_mn-stp,scl_mx+stp,stp)[::3])
