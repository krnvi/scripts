...
import numpy as np ; import pandas as pd ; import metpy.calc as mpcalc ; from metpy.units import units ; from scipy.interpolate import interp1d
...
surf_data=pd.DataFrame([[data['HGHT'].iloc[1],data['u_wind'].iloc[1],data['v_wind'].iloc[1]]])

surf_data.columns=['HGHT','u_wind','v_wind']
prof_data=pd.concat([data['HGHT'].iloc[2:],data['u_wind'].iloc[2:],data['v_wind'].iloc[2:]],axis=1)
new_data=pd.concat([surf_data,prof_data],axis=0)
new_data.HGHT.iloc[0]=0

w=np.zeros((new_data.shape[0]))
new_data['W']=w #pd.Series(np.zeros((new_data.shape[0])), index=new_data.index) 
...
f_rso = interp1d(new_data.HGHT,new_data.W,bounds_error=False) ; w_wind_new=f_rso(req_hgts)

new_data_new=pd.DataFrame({'HGHT':req_hgts,'u_wind':u_wind_new,'v_wind':v_wind_new,'W':w_wind_new})  
new_data_new=new_data_new[['HGHT','u_wind','v_wind','W']]
                         
new_data_new.to_csv('/home/vkvalappil/Data/modelWRF/WRF_SCM/force_sounding_'+date+'.csv',sep=' ',index=False) ;
