...
vis_data1=np.genfromtxt(vis_file,delimiter=',',dtype='S')[1:,:] ;   
vis_data=vis_data1.reshape(24,60,7)

vis_data1=vis_data.reshape(96,15,7)

met_data=np.genfromtxt(met_file,delimiter=',',dtype='S')[1:,:] ;   
met_vis=met_data[1:,8].astype(float)

h_data=vis_data1[:,:,1:4].astype(float).mean(axis=1)

vis=h_data[:,1]/1000 ; ecf=h_data[:,0]   ; 
time=vis_data1[:,0,0] ; 

#vis_1=vis_data1[:,1].astype(float)/1000


fig=plt.figure(figsize=(12,10),dpi=50); ax = fig.add_subplot(111,axisbg='white')
sns.set(style='ticks') ; pos  = np.arange(1,len(vis)+1,1) ;  pos1=np.arange(1,len(met_vis)+1,1) ;
...
#ax.plot(pos1,met_vis,'red',linestyle='-',linewidth=3.0,marker='o',markersize=2.0,\
...
#ax1.yaxis.set_ticks_position('right') ; ax1.set_yticks(np.arange(0,max(met_vis)+4,2))
#ax1.tick_params(axis='y', colors='red',labelsize=14) ; ax1.set_ylabel('Metar Visibility',color='red',fontsize=18) ; 

x_lim=pos ; ax.set_xticks(x_lim) ;  xTickMarks=time 
xtickNames = ax.set_xticklabels(xTickMarks) ; plt.setp(xtickNames, rotation=90, fontsize=8,family='sans-serif',color='black')
...
titl=' Visibility -  MI-Field Station' ; plt.title(titl,color='black',fontsize=22,y=1.00)
